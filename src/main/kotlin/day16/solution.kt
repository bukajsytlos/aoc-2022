...
fun main() {
    ...
    val valves = File("src/main/kotlin/day16/input.txt").readLines().associate {
    ...
    val distanceMatrix = buildDistanceMatrix(connections)

    val valvesWithPositiveFlowRate = valves.filter { it.value.flowRate > 0 }
    println(
        findMaximumPressureReleased(
            "AA",
            30,
            0,
    ...
    println((1 until (1 shl (valvesWithPositiveFlowRate.size - 1))).maxOf {
