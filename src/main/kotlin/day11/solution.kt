...
fun main() {
    ...
    val monkeys2 = monkeys.map { it.copy() }

    monkeys.evaluateRounds(20) { it / 3 }
    println(monkeys.determineMonkeyBusiness())

    val ambiguousWorryLevelConstant = monkeys.map { it.predicateDivisor }.reduce { acc, l -> acc * l }
    monkeys2.evaluateRounds(10000) { it % ambiguousWorryLevelConstant }
    println(monkeys2.determineMonkeyBusiness())
}
