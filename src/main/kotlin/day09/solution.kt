...
fun main() {
    ...
    val moves = lines.map { MoveCommand(it.substringBefore(" ").toDirection(), it.substringAfter(" ").toInt()) }

    val rope1 = Rope(List(2) { Knot(Position(0, 0)) })
    moves.forEach {
        rope1.moveHead(it)
    }
    println(rope1.visitedTailPositions.size)

    val rope2 = Rope(List(10) { Knot(Position(0, 0)) })
    moves.forEach {
        rope2.moveHead(it)
    }
    println(rope2.visitedTailPositions.size)
}
