...
fun main() {
    val monkeyReferences = File("src/main/kotlin/day21/input.txt").readLines().associate {
    ...
    if (rootMonkey.left.dependsOn("humn")) {
        println(resolve(rootMonkey.left, rootMonkey.right.evaluate()))
    } else {
        println(resolve(rootMonkey.right, rootMonkey.left.evaluate()))
    }
}
